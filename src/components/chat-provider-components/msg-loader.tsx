...
const MsgLoader = ({
  ...
  const { currChat, msgLoader, inputImgName, liveAgentSteps, chatMode } =
    geminiZustand();
  return (
    msgLoader && (
      <div key="loader" className="my-16 mt-10 fade-in-element">
        ...
        {chatMode === "agent" && liveAgentSteps.length > 0 && (
          <AgentSteps steps={liveAgentSteps} />
        )}
        <div className="w-full flex justify-end h-16 items-center"></div>
        <div
          id="new-chat"
          className="flex md:flex-row flex-col w-full items-start gap-4"
        >
